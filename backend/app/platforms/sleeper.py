...
class SleeperAdapter(PlatformAdapter):
    ...
    async def get_players_map(self) -> dict[str, dict]:
        """Fetch all NFL players from Sleeper. Cached per adapter instance."""
        if self._players_cache is not None:
            return self._players_cache
        resp = await self._client.get("/players/nfl", timeout=60.0)
    ...
    async def get_rosters(self, league_id: str) -> list[PlatformRosterEntry]:
        resp = await self._client.get(f"/league/{league_id}/rosters")
        resp.raise_for_status()
        data = resp.json()
        if not data:
            return []
    ...
    async def get_transactions(self, league_id: str, week: int) -> list[PlatformTransaction]:
        resp = await self._client.get(f"/league/{league_id}/transactions/{week}")
        resp.raise_for_status()
        data = resp.json()
        if not data:
            return []
